...
const getAllTasksForAdmin = asyncHandler(async (req, res) => {
    if (!req.user.isAdmin) {
        return res.status(403).json({ status: false, message: "Access denied" });
    }

    try {
        // Fetch all tasks and populate necessary fields for detailed overview
        const tasks = await Task.find({})
            .populate('team', 'name email')
            .populate('activities.by', 'name');

        // Format the tasks for detailed progress view
        const taskDetails = tasks.map(task => {
            return {
                taskId: task._id,
                title: task.title,
                status: task.stage,  // Assuming 'stage' has values like 'completed', 'pending', etc.
                completion: task.stage.toLowerCase() === 'completed', // Boolean flag for completion
                priority: task.priority,
                assignedTo: task.team.map(user => ({
                    name: user.name,
                    email: user.email,
                    isTaskComplete: task.stage.toLowerCase() === 'completed' // Individual completion status based on task stage
                })),
                dueDate: task.date.toDateString(),
        ...
        res.json({ status: true, tasks: taskDetails });
    } catch (error) {
